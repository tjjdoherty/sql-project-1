...
-- Question 3: Is there any pattern in the types (product categories) of products ordered from visitors in each city and country?
		...
		-- Let's use the products_clean table and units ordered to see the most popular categories before introducing city/country
			...
			SELECT DISTINCT(sesh.v2productname), p.orderedquantity, sesh.v2productcategory
			FROM products_clean p
			JOIN all_sessions_clean sesh ON p.sku = sesh.product_sku
			ORDER BY sesh.v2productname

				
		-- Now we'll try to add city, country and transactions
			SELECT orderedquantity, name from products_clean WHERE name = 'Kick Ball'
			SELECT v2productname FROM all_sessions_clean WHERE v2productname LIKE '%Kick Ball%'
...
-- Question 5: Can we summarize the impact of revenue generated from each city/country?
	...
	SELECT city, country FROM all_sessions_clean
